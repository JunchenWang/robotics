...
iiwa = convert_robot_tree(robot);
iiwa.TCP = [eye(3), [0, 0, 0.213]'; 0 0 0 1];
...
[B, T0] = jacobian_matrix(iiwa, zeros(1,n));
Jb2 = jacobian(B', q, 'b');
T2 = getTransform(robot, q, 'flange');
% T2 = getTransform(robot, q, 'iiwa_link_ee_kuka');
T2 = T2 * iiwa.TCP;
