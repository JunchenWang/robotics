function tao = torque_controller(robot, Td, w_v_d, alpha_a_d, Kp, Ki, Kd, Bn, Kn, kesai, freq, t, y)
% Xd is desired motion in task space
% tao_ext is the external torque applied to environment
persistent sum;
persistent pre_t;
persistent pre_xe;
if isempty(sum)
...
xdd = dAdXe * Vd + AdXe * dVd + Kp * xe + Ki*sum + Kd * Ve;
a1 = pinv_J_x(Jb, M, xdd - dJb * qd);
% N = eye(7) - (M \ Jb') * ((Jb * (M \ Jb')) \ Jb);
qe = q0 - q;
qed = -qd;
a2 = null_proj(Jb, M, M \ (Bn * qed + Kn * qe));
tao = inverse_dynamics(robot, q, qd, a1 + a2, zeros(6,1));
pre_t = t;
pre_xe = xe;
% disp(tao);
