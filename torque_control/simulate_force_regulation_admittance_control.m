function simulate_force_regulation_admittance_control
...
MassMatrix = @(t, y) blkdiag(eye(n), mass_matrix(robot, y(1:n)),eye(n), eye(12));
...
B = 2*sqrt(K .* M);
S = [0,0,0,0,0,1]';
Fd = [0, 0, 0, 0, 0, 10]'; % desired force in body frame
admittance = @(t, y, fext) addmittance_controller(robot2, motion_planner, M, B, K, t, y, fext, S, Fd);
...
function [Td, vel, acc, state] = addmittance_controller(robot, motion_planner, M, B, K, t, y, fext, S, Fd)
    n = robot.dof;
    % X = forward_kin_general(robot, y(1:n));
    % X(1:3,4) = zeros(3,1);
    % Fd = adjoint_T(X)' * Fd;
    Fex = -fext(:,end) - S .* Fd;
    K = K .* (ones(6,1) - S);
...
if real_f < 0
    ...
    F(:,end) = adjoint_T(X)' * [1000 * cross(z, [0,0,-1]');0;0;-real_f]; % from s to b
end
end
