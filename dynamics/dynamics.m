...
gt2 = velocity_torque(mass, inertia, A, M, q, qd);
disp(norm(gt1'-gt2));
% F_ME = 100 * rand(6,1);
% fext = externalForce(robot,'iiwa_link_ee_kuka',-F_ME,q);
% % q = [10, 20, 30, 40, 50, 60, 70] / 180 * pi;
% % q = zeros(7,1);
% tao = inverse_dynamics(mass, inertia, A, M, ME, q, qd, qdd, F_ME);
% qdd_x1 = forward_dynamics(mass, inertia, A, M, ME, q, qd, tao, F_ME);
...
[Jb, T] = jacobian_matrix(A, M, ME, q);
[B, T0] = jacobian_matrix(A, M, ME, zeros(1,7));
Jb2 = jacobian(B', q, 'b');
